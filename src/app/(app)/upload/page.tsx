...
export default function UploadPage() {
  ...
  async function uploadSelectedFiles() {
    ...
    try {
      ...
      // Server-side proxy upload (avoids CORS with MinIO; more reliable). All in parallel, single state update at end.
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-8 text-center">
        <h1 className="font-display text-3xl font-bold text-white sm:text-4xl">Upload Resumes</h1>
        <p className="mx-auto mt-3 max-w-2xl font-sans text-base text-slate-300">
          ...
          {[
            ...
            <div key={step.num} className="flex flex-1 items-center">
              <div className="flex flex-col items-center">
                ...
                <p className="mt-2 hidden text-xs font-semibold text-slate-300 sm:block md:text-sm">
                  {step.label}
                </p>
              </div>
              {idx < 2 && (
                <div className="mx-2 h-1 flex-1 rounded-full bg-slate-600 sm:mx-4">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      currentStep > step.num
                        ? "w-full bg-gradient-to-r from-indigo-600 to-cyan-500"
                        : "w-0"
                    }`}
                  />
                </div>
      ...
      <div className="space-y-6">
        ...
        <div className="animate-fade-in overflow-hidden rounded-2xl border border-slate-700 bg-slate-800/70 backdrop-blur transition hover:border-slate-600">
          <button
            type="button"
            onClick={() => setShowJdInput(!showJdInput)}
            className="flex w-full items-center justify-between p-6 text-left transition-colors hover:bg-slate-700/50"
          ...
          {showJdInput && (
            <div className="border-t border-slate-700 bg-slate-700/40 p-6">
              <textarea
                className="w-full resize-y rounded-xl border border-slate-600 bg-slate-700/80 px-4 py-3 font-sans text-sm text-white placeholder:text-slate-400 transition focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                rows={6}
                value={contextText}
                ...
                disabled={status === "uploading" || status === "processing"}
              />
              <div className="mt-3 flex items-start gap-2 font-sans text-xs text-slate-400">
                ...
                <span>
                  <strong className="text-slate-300">Tip:</strong> Paste the full JD or just the &quot;Must have&quot; requirements. The more detail you provide, the better the analysis.
                </span>
              </div>
            </div>
          )}
        ...
        {/* Step 2: Upload Files */}
        <div className="animate-fade-in rounded-2xl border border-slate-700 bg-slate-800/70 backdrop-blur transition hover:border-slate-600">
          <div className="p-6">
            <div className="mb-6 flex items-center gap-4">
              ...
              <div>
                <h3 className="font-display text-lg font-bold text-white">Step 2: Upload Resumes</h3>
                <p className="mt-1 font-sans text-sm text-slate-300">
            ...
            >
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-cyan-500/5 opacity-0 transition-opacity group-hover:opacity-100" />
              ...
              <p className="mt-1 font-sans text-sm text-slate-400">or click to browse from your computer</p>

              <button
                type="button"
                className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                onClick={(e) => {
                  e.stopPropagation();
                  openFilePicker();
                }}
              ...
              <div className="mt-6 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {!isUploaded && status !== "uploading" && (
                    <button
                      type="button"
                      className="font-sans text-xs font-medium text-indigo-300 hover:text-indigo-200"
                      onClick={() => {
                        resetFlow();
                        setSelectedFiles([]);
                      }}
                ...
                <div className="space-y-2">
                  {selectedFiles.map((file) => {
                    ...
                    return (
                      <div
                        key={file.name}
                        className="flex items-center gap-3 rounded-xl border border-slate-600 bg-slate-700/60 p-3 transition-all"
                      >
                        <div
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-sans text-sm font-medium text-white">{file.name}</p>
                          <p className="font-sans text-xs text-slate-400">
                            {(file.size / 1024).toFixed(1)} KB
                          </p>
                        </div>
                        <span
                          className={`flex-shrink-0 rounded-full px-2.5 py-1 font-sans text-xs font-semibold ${
                            isDone
            ...
            {/* Error */}
            {error && (
              <div className="mt-4 flex animate-fade-in items-start gap-3 rounded-xl border border-red-500/30 bg-red-500/10 p-4">
            ...
            {/* Success Message */}
            {isUploaded && (
              <div className="mt-4 flex animate-fade-in items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4">
                ...
                <div className="flex-1">
                  <p className="font-sans text-sm font-semibold text-emerald-300">Files uploaded successfully!</p>
                  <p className="mt-1 font-sans text-xs text-emerald-400/90">
        ...
        {selectedFiles.length > 0 && (
          <div className="animate-fade-in rounded-2xl border border-slate-700 bg-slate-800/70 p-6 backdrop-blur transition hover:border-indigo-500/30">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
              ...
              <button
                type="button"
                className="group relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 px-8 py-4 text-base font-semibold text-white shadow-lg shadow-indigo-500/25 transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            {status === "uploading" && (
              <div className="mt-6">
                <div className="h-2 overflow-hidden rounded-full bg-slate-600">
                  <div className="h-full animate-pulse rounded-full bg-gradient-to-r from-indigo-600 to-cyan-500" style={{ width: "60%" }} />
                </div>
                <p className="mt-2 text-center font-sans text-xs font-medium text-slate-400">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-slate-700 bg-slate-800/70 p-5 transition hover:border-slate-600">
            ...
            <h4 className="font-display font-bold text-white">Instant Analysis</h4>
          ...
          <div className="rounded-xl border border-slate-700 bg-slate-800/70 p-5 transition hover:border-slate-600">
            ...
            <h4 className="font-display font-bold text-white">Structured Insights</h4>
            <p className="mt-1 font-sans text-xs text-slate-300">
              Get decision-ready analysis and hiring insights
            </p>
          </div>
          <div className="rounded-xl border border-slate-700 bg-slate-800/70 p-5 transition hover:border-slate-600">
            ...
            <h4 className="font-display font-bold text-white">Secure & Private</h4>
            <p className="mt-1 font-sans text-xs text-slate-300">
